...
(function(blocks, element, blockEditor, components, i18n) {
    ...
    registerBlockType('derleiti/ai-content', {
        ...
        edit: function(props) {
            ...
            useEffect(() => {
                // Only fetch if we haven't already loaded providers
            ...
            const fetchProviders = () => {
                ...
                .then(data => {
                    if (data.success) {
                        // Create provider options for select control
                        const providers = [];
                        ...
                        // If we have an active provider from the API, use it
            ...
            const generateContent = () => {
                ...
                // For REST API requests we need to use the correct API endpoint
            ...
            // When prompt changes and autoUpdate is enabled, generate content
            useEffect(() => {
                if (attributes.autoUpdate && attributes.prompt.trim() && !isGenerating) {
                    // Use a debounce function to avoid too many requests
                ...
                <div key="content" className="derleiti-ai-content-block">
                    <div className="derleiti-ai-prompt-container">
                        ...
                        <div className="derleiti-ai-actions">
                            ...
                            >
                                {isGenerating ? derleitiBlocksData.strings.generating : 
                                 attributes.generatedContent ? derleitiBlocksData.strings.regenerateContent : 
                                 derleitiBlocksData.strings.generateContent}
                            </Button>
                            
                            <div className="derleiti-ai-settings-summary">
                                <span className="derleiti-ai-setting">{__('Typ:', 'derleiti-plugin')} {contentTypeOptions.find(o => o.value === attributes.contentType)?.label}</span>
                                <span className="derleiti-ai-setting">{__('Ton:', 'derleiti-plugin')} {toneOptions.find(o => o.value === attributes.tone)?.label}</span>
                                <span className="derleiti-ai-setting">{__('Länge:', 'derleiti-plugin')} {lengthOptions.find(o => o.value === attributes.length)?.label}</span>
                            </div>
                        </div>
        ...
        save: function(props) {
            // The generated content is rendered on the server side
            return null;
        }
    });
})(
