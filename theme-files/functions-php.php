    ...
    private function get_pattern_content($pattern_name) {
        ...
        switch ($pattern_name) {
            case 'hero-section':
                ...
                <div class="wp-block-group alignfull has-background-color has-secondary-background-color has-text-color has-background" style="padding-top:var(--wp--preset--spacing--80);padding-bottom:var(--wp--preset--spacing--80)">
                <!-- wp:heading {"textAlign":"center","level":1,"fontSize":"xx-large"} -->
                <h1 class="has-text-align-center has-xx-large-font-size">Welcome to Derleiti Modern</h1>
                <!-- /wp:heading -->
    ...
    public function render_ai_content_block($attributes, $content) {
        ...
        if ($theme_integration && class_exists('Derleiti_AI_Integration') && current_theme_supports('derleiti-ai-integration')) {
            ...
            if (method_exists($ai, 'process_theme_content')) {
                // process_theme_content() sollte vier Parameter akzeptieren: content, content_type, content_style, custom_classes
...
// Instantiate and initialize the blocks manager
$derleiti_blocks_manager = new Derleiti_Blocks_Manager();
$derleiti_blocks_manager->init();
