...
function derleiti_plugin_activate(): void {
    ...
    wp_mkdir_p(WP_CONTENT_DIR . '/cache/derleiti-plugin');
...
function derleiti_features_shortcode($atts): string {
    ...
    echo '<div' . $id_attr . $class_attr . ' data-derleiti-feature="' . esc_attr($atts['feature']) . '">';
    ...
    switch ($atts['feature']) {
        case 'layout':
            if (file_exists(DERLEITI_PLUGIN_PATH . 'templates/layout-builder.php')) {
                ...
                _e('Layout-Builder-Template nicht gefunden', 'derleiti-plugin');
            }
            break;
        case 'ai':
            if (file_exists(DERLEITI_PLUGIN_PATH . 'templates/ai-content.php')) {
                ...
                _e('KI-Content-Template nicht gefunden', 'derleiti-plugin');
            }
            break;
        case 'gallery':
            if (file_exists(DERLEITI_PLUGIN_PATH . 'templates/enhanced-gallery.php')) {
                ...
                _e('Gallery-Template nicht gefunden', 'derleiti-plugin');
            }
            break;
        case 'analytics':
            if (file_exists(DERLEITI_PLUGIN_PATH . 'templates/analytics.php')) {
                ...
                _e('Analytics-Template nicht gefunden', 'derleiti-plugin');
            }
            break;
        default:
            _e('Feature nicht gefunden', 'derleiti-plugin');
    }
 ...
 */
function derleiti_plugin_clear_cache_endpoint(): array {
    if (function_exists('derleiti_clear_all_caches')) {
        derleiti_clear_all_caches();
        return [
            'success' => true,
...
function derleiti_plugin_admin_notices(): void {
    ...
    if (get_transient('derleiti_plugin_activated')) {
        ...
        <div class="notice notice-success is-dismissible">
            <h3><?php _e('Willkommen beim Derleiti Modern Theme Plugin!', 'derleiti-plugin'); ?></h3>
            <p><?php _e('Vielen Dank, dass Sie sich für unser Plugin entschieden haben. Hier sind einige nützliche Links, um zu beginnen:', 'derleiti-plugin'); ?></p>
            <ul style="list-style-type: disc; padding-left: 20px;">
                <li><a href="<?php echo admin_url('admin.php?page=derleiti-plugin'); ?>"><?php _e('Plugin-Einstellungen', 'derleiti-plugin'); ?></a></li>
                <li><a href="<?php echo admin_url('admin.php?page=derleiti-help'); ?>"><?php _e('Hilfe und Dokumentation', 'derleiti-plugin'); ?></a></li>
            </ul>
        </div>
        <?php
    }
        ...
        if (version_compare($previous_version, '0.0.0', '>')) {  // Nur anzeigen, wenn es ein Update ist, nicht bei Neuinstallation
            ...
            <div class="notice notice-info is-dismissible">
                <p>
                    <?php 
                    echo sprintf(
                        __('Das Derleiti Modern Theme Plugin wurde auf Version %s aktualisiert. <a href="%s">Was ist neu?</a>', 'derleiti-plugin'),
                        $current_version,
                        admin_url('admin.php?page=derleiti-help&tab=changelog')
                    ); 
                    ?>
                </p>
            </div>
...
function derleiti_debug_info(): void {
    ...
    echo '<p style="margin:5px 0;">Version: ' . DERLEITI_PLUGIN_VERSION . '</p>';
    echo '<p style="margin:5px 0;">Memory: ' . $memory_usage . ' MB</p>';
    echo '<p style="margin:5px 0;">Queries: ' . $queries . ($query_time ? ' (' . $query_time . 's)' : '') . '</p>';
    echo '<p style="margin:5px 0;">PHP: ' . PHP_VERSION . '</p>';
    echo '</div>';
}
