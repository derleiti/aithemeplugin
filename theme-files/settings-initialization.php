...
class Derleiti_Settings_Manager {
    ...
    private function check_and_create_settings() {
        ...
        // Verify and create default settings for each category
    ...
    private function ensure_category_settings($category, $settings) {
        ...
        foreach ($settings as $setting_name => $default_value) {
            $exists = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM $table_name WHERE category = %s AND setting_name = %s",
                                                    $category,
                                                    $setting_name
            ));
            ...
            if (!$exists) {
                $wpdb->insert(
                    ...
                    [
                        ...
                        'setting_value' => is_array($default_value) ? maybe_serialize($default_value) : $default_value,
                    ],
                    ['%s', '%s', '%s']
                );
            }
    ...
    private function migrate_legacy_plugin_settings() {
        ...
        // Weitere Migrationslogik hier hinzufügen.
    }
    ...
    public function get_setting($category, $setting_name, $default = null) {
        ...
        $value = $wpdb->get_var($wpdb->prepare(
            "SELECT setting_value FROM $table_name WHERE category = %s AND setting_name = %s",
            $category,
            $setting_name
        ));
    ...
    public function get_category_settings($category) {
        ...
        $results = $wpdb->get_results($wpdb->prepare(
            "SELECT setting_name, setting_value FROM $table_name WHERE category = %s",
            $category
        ), ARRAY_A);
